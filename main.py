...
@app.get("/v1/team/active")
async def list_active(company: dict = Depends(require_admin)):
    result = (
        supabase.table("company_users")
        .select("user_id, role")
        .eq("company_id", company["company_id"])
    ...
    members = [
        {"user_id": row["user_id"], "email": get_user_email(row["user_id"]), "role": row["role"]}
...
async def remove_member(request: TeamActionRequest, company: dict = Depends(require_admin)):
    if request.user_id == company["user_id"]:
